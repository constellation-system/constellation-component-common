...
#[derive(Debug)]
pub enum MulticastBusRunError<Acquire, MsgCodec, Stream, Refresh> {
        ...
        + Sync
{
    ...
    >{
        ...
        ctx.far_channel_registry()
            .acquire_all(&mut ctx)
            .map_err(|err| MulticastBusRunError::Acquire { err: err })?;
        ...
        let msg_codec = MsgCodec::create(MsgCodec::Param::default())
            .map_err(|err| MulticastBusRunError::MsgCodec { err: err })?;
        let listener =
    ...
    /// cleanup object.
    pub fn start(self) -> MulticastBusCleanup {
        let MulticastBus {
            pull,
            push,
            reporter,
...
impl<Acquire, MsgCodec, Stream, Refresh> Display
    for MulticastBusRunError<Acquire, MsgCodec, Stream, Refresh>
where
    Acquire: Display,
    MsgCodec: Display,
    Stream: Display,
    Refresh: Display
{
    fn fmt(
        ...
        match self {
            MulticastBusRunError::Acquire { err } => err.fmt(f),
            MulticastBusRunError::MsgCodec { err } => err.fmt(f),
            MulticastBusRunError::Stream { err } => err.fmt(f),
            MulticastBusRunError::Refresh { err } => err.fmt(f)
        }
    }
}
